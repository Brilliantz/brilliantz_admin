    ...
    <v-card>
      <v-container fluid>
        <v-row class="pt-3 px-6">
          <v-col cols="12">
            <p class="text-grey mb-0 mt-4">Gambar Pertanyaan</p>
            <quill-editor-image
                ref="editorImage"
                @syncImageSoal="syncImageSoal"
            />
            <p class="text-grey">Text Pertanyaan</p>
            <quill-editor-custom
...
export default {
  ...
  methods: {
    resetEditorImageData(){
      this.$refs.editorImage.onSyncParentImage(this.soal.gambar_soal)
    },
    syncImageSoal(image){
    ...
    onHandlerImageUpload(image = ''){
      ...
      uploadTask.then(snapshot => {
        snapshot.ref.getDownloadURL().then(res => {
          this.soal.gambar_soal =res
          this.handleSaveToFirestore()
        })
      })
